...
/// single shared <see cref="WebSocketClient"/>, and layers REST-based gap recovery on top of
/// the user-scoped channels (fills, orders, fundings) so events that fall in the disconnect
/// window aren't silently dropped.
/// </summary>
internal sealed class Streams(WebSocketClient ws, InfoClient info, HyperLiquidClientOptions options) : IStreams
{
    // ─── Public channels ─────────────────────────────────────────────────────
    ...
    public IAsyncEnumerable<CandleUpdate> CandlesAsync(string symbol, Interval interval, CancellationToken ct = default)
        => ws.SubscribeAsync<CandleUpdate>(
            subscribePayload: new HlMap().Add("type", "candle").Add("coin", symbol).Add("interval", Mapper.SerializeInterval(interval)),
            subscriptionKey:  $"candle:{symbol}:{Mapper.SerializeInterval(interval)}",
            parser:           ParseCandle,
            ct:               ct);
    ...
    public IAsyncEnumerable<OrderUpdate> MyOrdersAsync(CancellationToken ct)
    {
        ...
        return StreamGapFill.WithRecoveryAsync(
            live, ws,
            fetchSince: async (sinceMs, c) =>
            {
                ...
                return raw
                    ...
                    .Select(o => new OrderUpdate(Mapper.Map(o.Order, Mapper.ParseOrderStatus(o.Status), o.StatusTimestamp)))
                    .ToList();
            },
    ...
    public IAsyncEnumerable<FillUpdate> MyFillsAsync(CancellationToken ct)
    {
        ...
        return StreamGapFill.WithRecoveryAsync(
            ...
            fetchSince: async (sinceMs, c) =>
            {
                ...
                return raw.Select(f => new FillUpdate(Mapper.Map(f))).ToList();
            },
        ...
        return StreamGapFill.WithRecoveryAsync(
            ...
            fetchSince: async (sinceMs, c) =>
            {
                ...
                foreach (var f in arr.EnumerateArray())
                {
                    ...
                    var time = Mapper.T(f.GetProperty("time").GetInt64());
    ...
    private static IEnumerable<TradeUpdate> ParseTrades(JsonElement data)
    {
        ...
        foreach (var t in data.EnumerateArray())
        {
            var trade = new PublicTrade(
                ...
                Side:    Mapper.ParseSide(t.GetProperty("side").GetString()!),
                Time:    Mapper.T(t.GetProperty("time").GetInt64()),
    ...
    private static IEnumerable<OrderBookUpdate> ParseOrderBook(JsonElement data, int depth)
    {
        ...
        var time = Mapper.T(data.GetProperty("time").GetInt64());
    ...
    private static IEnumerable<CandleUpdate> ParseCandle(JsonElement data)
    {
        ...
        var candle = new Candle(
            ...
            Interval:   Mapper.ParseInterval(data.GetProperty("i").GetString()!),
            OpenTime:   Mapper.T(data.GetProperty("t").GetInt64()),
            CloseTime:  Mapper.T(data.GetProperty("T").GetInt64()),
    ...
    private static IEnumerable<FillUpdate> ParseUserFills(JsonElement data)
    {
        ...
        foreach (var f in fillsEl.EnumerateArray())
        {
            ...
            if (raw is null) continue;
            yield return new FillUpdate(Mapper.Map(raw));
        }
    }
