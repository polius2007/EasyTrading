...
/// </remarks>
internal sealed class MetaCache(InfoClient info) : IDisposable
{
    ...
    /// </summary>
    public async Task<AssetInfo> GetAssetInfoAsync(string symbol, CancellationToken ct = default)
    {
    ...
    private async Task EnsureLoadedAsync(CancellationToken ct)
    {
        ...
        try
        {
            ...
            var assets = new Dictionary<string, AssetInfo>(meta.Universe.Count, StringComparer.OrdinalIgnoreCase);
            var perpSyms = new List<string>(meta.Universe.Count);
            for (var i = 0; i < meta.Universe.Count; i++)
            {
                var a = meta.Universe[i];
                assets[a.Name] = new AssetInfo(AssetId: i, SzDecimals: a.SzDecimals, IsSpot: false);
                perpSyms.Add(a.Name);
            }
            ...
            foreach (var pair in spot.Universe)
            {
                ...
                var szDecimals = baseTokenIndex >= 0 && tokensByIndex.TryGetValue(baseTokenIndex, out var tok)
                    ? tok.SzDecimals
                    : 0;
                assets[pair.Name] = new AssetInfo(AssetId: 10_000 + pair.Index, SzDecimals: szDecimals, IsSpot: true);
            }
