...
/// signing (delegating to <see cref="Signer"/>), nonce allocation, request envelope assembly,
/// HTTP transport, and error mapping into typed <see cref="ExchangeApiException"/> subclasses.
/// </summary>
internal sealed class ExchangeClient
{
    ...
    public ExchangeClient(HttpClient http, HyperLiquidClientOptions options, Nonce nonce)
    {
    ...
    public async Task<JsonElement> SendL1Async(HlMap action, long? expiresAfter, CancellationToken ct)
    {
        ...
        var signature = Signer.SignL1Action(action, vaultAddress, nonce, expiresAfter, isMainnet, creds.PrivateKey);
    ...
    private async Task<JsonElement> PostAsync(HlMap envelope, CancellationToken ct)
    {
        ...
        try
        {
            result = await Http.PostJsonAsync(_http, _exchangeUrl, json, _options.RetryPolicy, ct).ConfigureAwait(false);
        }
