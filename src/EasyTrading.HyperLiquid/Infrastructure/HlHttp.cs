...
/// <see cref="HlInfoClient"/> and <see cref="HlExchangeClient"/> use this so retries,
...
internal static class HlHttp
{
    ...
    /// </summary>
    public static async Task<HlHttpResult> PostJsonAsync(
        HttpClient http,
        Uri uri,
        string jsonBody,
        ...
        CancellationToken ct)
    {
        ...
        for (var attempt = 1; attempt <= attempts; attempt++)
        {
            ...
            try
            {
                ...
                return new HlHttpResult(body, status, reason);
            }
        ...
        throw lastException ?? new InvalidOperationException("HlHttp retry loop exited without a result.");
    }
...
/// <summary>Result of a (possibly retried) HTTP POST through <see cref="HlHttp"/>.</summary>
...
internal readonly record struct HlHttpResult(string Body, HttpStatusCode StatusCode, string? ReasonPhrase);
