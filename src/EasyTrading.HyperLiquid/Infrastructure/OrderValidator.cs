...
internal static class OrderValidator
{
    ...
    /// <param name="info">Asset metadata from <see cref="MetaCache.GetAssetInfoAsync"/>.</param>
    /// <param name="reduceOnly">Whether this order is reduce-only — the min-notional check is skipped if so.</param>
    public static void Validate(string symbol, decimal price, decimal size, AssetInfo info, bool reduceOnly)
    {
        if (size <= 0m)
    ...
    private static void ValidatePrice(string symbol, decimal price, AssetInfo info)
    {
        if (price <= 0m)
