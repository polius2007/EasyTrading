...
/// <see cref="Signer"/>, and attaches the signature.
/// </summary>
internal sealed class RestClient
{
    ...
    public RestClient(HttpClient http, AsterClientOptions options, Nonce nonce)
    {
        ...
        CancellationToken ct)
    {
        ...
        var signature = Signer.Sign(msg, creds.PrivateKey);
    ...
    private async Task<string> SendRawAsync(HttpMethod method, Uri url, HttpContent? content, CancellationToken ct)
    {
        ...
        try
        {
            result = await Http.SendAsync(_http, method, url, content, _options.RetryPolicy, ct).ConfigureAwait(false);
        }
    ...
    private static void ThrowFromStatus(HttpResult result)
    {
