...
/// </summary>
internal sealed class AsterMetaCache(AsterRestClient rest) : IDisposable
{
    private readonly SemaphoreSlim _lock = new(1, 1);

    public void Dispose() => _lock.Dispose();

    private Dictionary<string, AsterSymbolInfo>? _symbols;

    /// <summary>Resolve a symbol to its filters. Throws <see cref="ExchangeApiException"/> if unknown.</summary>
    public async Task<AsterSymbolInfo> GetAsync(string symbol, CancellationToken ct = default)
    {
    ...
    private async Task EnsureLoadedAsync(CancellationToken ct)
    {
        ...
        try
        {
            ...
            var map = new Dictionary<string, AsterSymbolInfo>(raw.Symbols.Count, StringComparer.OrdinalIgnoreCase);
            ...
            foreach (var s in raw.Symbols)
            {
                ...
                map[s.Symbol] = new AsterSymbolInfo(
                    Symbol:           s.Symbol,
                    Status:           s.Status,
...
internal sealed record AsterSymbolInfo(
    string Symbol,
    string Status,
    int PricePrecision,
