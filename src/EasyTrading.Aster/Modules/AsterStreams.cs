...
/// WebSocket (<see cref="AsterWebSocketClient"/>). User-scoped streams require a <c>listenKey</c>
/// obtained from REST and bound to a separate WebSocket — see <see cref="EnsureUserSocketAsync"/>.
/// </summary>
internal sealed class AsterStreams : IStreams, IAsyncDisposable
{
    private readonly AsterWebSocketClient _market;
    private readonly AsterRestClient _rest;
    ...
    private AsterWebSocketClient? _user;
    private string? _listenKey;
    private CancellationTokenSource? _userKeepaliveCts;

    public AsterStreams(AsterWebSocketClient marketClient, AsterRestClient rest, AsterClientOptions options)
    {
    ...
    private async Task EnsureUserSocketAsync(CancellationToken ct)
    {
        ...
        try
        {
            ...
            _user = new AsterWebSocketClient(
                async ctk =>
                {
    ...
    private static IEnumerable<TradeUpdate> ParseAggTrade(JsonElement data, string symbol)
    {
        ...
        yield return new TradeUpdate(new PublicTrade(symbol, price, qty, side, AsterMapper.ToDt(time), aggId));
    }
    ...
    private static IEnumerable<OrderBookUpdate> ParseDepth(JsonElement data, string symbol)
    {
        ...
        yield return new OrderBookUpdate(symbol, AsterMapper.ToDt(time), bids, asks, IsSnapshot: false);
    }
    ...
    private static IEnumerable<CandleUpdate> ParseKline(JsonElement data)
    {
        ...
        yield return new CandleUpdate(new Candle(
            ...
            OpenTime:   AsterMapper.ToDt(t),
            CloseTime:  AsterMapper.ToDt(T),
    ...
    private static IEnumerable<BboUpdate> ParseBookTicker(JsonElement data, string symbol)
    {
        ...
        yield return new BboUpdate(symbol, bidPx, bidSz, askPx, askSz, AsterMapper.ToDt(time));
    }
    ...
    private static IEnumerable<OrderUpdate> ParseOrderUpdate(JsonElement data)
    {
        ...
        yield return new OrderUpdate(new Order(
            ...
            ClientOrderId: cloid,
            Symbol:        sym,
            Side:          AsterMapper.ParseSide(side),
            OrderType:     AsterMapper.ParseType(type),
            Price:         price > 0 ? price : null,
            TriggerPrice:  trigger > 0 ? trigger : null,
            Size:          qty,
            FilledSize:    filled,
            TimeInForce:   AsterMapper.ParseTif(tif),
            ReduceOnly:    reduce,
            Status:        AsterMapper.ParseStatus(status),
            CreatedAt:     AsterMapper.ToDt(time),
            UpdatedAt:     AsterMapper.ToDt(time)));
    }
    ...
    private static IEnumerable<FillUpdate> ParseFillUpdateFromOrder(JsonElement data)
    {
        ...
        var sym    = o.GetProperty("s").GetString() ?? string.Empty;
        var side   = AsterMapper.ParseSide(o.GetProperty("S").GetString() ?? "BUY");
    ...
    private static IEnumerable<NotificationUpdate> ParseMarginCall(JsonElement data)
    {
        ...
        yield return new NotificationUpdate("MARGIN_CALL — Aster reports your margin level is low; positions may be auto-liquidated.", AsterMapper.ToDt(time));
    }
