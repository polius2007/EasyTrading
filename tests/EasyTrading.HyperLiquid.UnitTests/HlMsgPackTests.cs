...
public sealed class HlMsgPackTests
{
    ...
    public void Preserves_map_insertion_order()
    {
        ...
        var a = MsgPack.Pack(new HlMap().Add("a", 1).Add("b", 2));
        var b = MsgPack.Pack(new HlMap().Add("b", 2).Add("a", 1));
        Assert.NotEqual(a, b);
    }
    ...
    {
        Assert.Equal(new byte[] { 0xc3 }, MsgPack.Pack(true));
        Assert.Equal(new byte[] { 0xc2 }, MsgPack.Pack(false));
        Assert.Equal(new byte[] { 0xc0 }, MsgPack.Pack(null));
    }

    [Fact]
    public void Encodes_positive_fixint_boundary()
    {
        Assert.Equal(new byte[] { 0x00 }, MsgPack.Pack(0));
        Assert.Equal(new byte[] { 0x7f }, MsgPack.Pack(127));
    }
    ...
    public void Encodes_uint8_above_fixint_range()
    {
        ...
        Assert.Equal(new byte[] { 0xcc, 0x80 }, MsgPack.Pack(128));
        Assert.Equal(new byte[] { 0xcc, 0xff }, MsgPack.Pack(255));
    }
    ...
    public void Encodes_uint16_and_uint32_at_boundaries()
    {
        ...
        Assert.Equal(new byte[] { 0xcd, 0x01, 0x00 }, MsgPack.Pack(256));
        // 65536 → uint32 (0xce) + 0x00010000
        Assert.Equal(new byte[] { 0xce, 0x00, 0x01, 0x00, 0x00 }, MsgPack.Pack(65536));
    }
    ...
    public void Encodes_negative_fixint_and_int8()
    {
        ...
        Assert.Equal(new byte[] { 0xff }, MsgPack.Pack(-1));
        // -32 is the lower bound of negative fixint → 0xe0
        Assert.Equal(new byte[] { 0xe0 }, MsgPack.Pack(-32));
        // -33 needs int8 → 0xd0 0xdf
        Assert.Equal(new byte[] { 0xd0, 0xdf }, MsgPack.Pack(-33));
    }

    [Fact]
    public void Encodes_fixarray()
    {
        var bytes = MsgPack.Pack(new object?[] { 1, true, "x" });
